...
// Helper function: Apply softmax to rows of a matrix
...
// Helper function: Apply layer normalization
...
public:
    ...
    vector<vector<float>> forward(const vector<vector<float>>& input, Timings& timings) {
        ...
        timings.mha_forward += duration_cast<microseconds>(end - start).count() / 1000.0; // ms
...
public:
    ...
    vector<vector<float>> forward(const vector<vector<float>>& input, Timings& timings) {
        ...
        // Total time for Feed Forward
        auto end = high_resolution_clock::now();
        timings.feed_forward += duration_cast<microseconds>(end - start).count() / 1000.0; // ms
